...
int main() {
    ...
    double lambda[4]={0.1,0.1,0.1,0.1};//設0.1進來任務次數少, 設0.9進來任務次數多
    double p[4]={0.3,0.3,0.3,0.3};//設0.1 log0.9=-0.045 產生封包數多, 設0.9 log0.1=-1 ,產生封包數少
    ...
    for(i=1;i<infinite;i++){
        ...
        while(q>0 || w>0){
            ...
            //8printf("%d\t%d\t%d\t%d\n",cost[s],cost[t],cost[u],cost[v]);
            //自已的演算法 等最久的任務先做
            if((cost[s]>cost[t] && cost[s]>cost[u]) || (cost[v]>cost[t] && cost[v]>cost[u])){
            ...
            else if((cost[t]>cost[s] && cost[t]>cost[v]) || (cost[u]>cost[s] && cost[u]>cost[v])){
